...
Images are ranked via a round-robin tournament — every image is scored against
every other image, Elo and Bradley-Terry MLE ratings are updated after each
match, and final results are exported to a CSV and JSON.
...
This runs 50 images × 1225 pairwise matches and writes:
- `results/evaluation_scores.csv` — per-image ranked metrics table
...
| `elo_rating` | Elo score after all matches (higher = better) |
| `bt_rating` | Bradley-Terry MLE score (used for final rank) |
...
Four PNG charts generated by `python main.py visualize`:
- `heatmap.png` — pairwise win/loss matrix
- `rankings.png` — final rankings bar chart
- `progression.png` — Elo rating history per image
- `distribution.png` — score distribution histogram
...
Each match takes ~60 s on Mac MPS, ~5–10 s on GPU. For 50 images (1225 matches):
- Mac MPS: ~20 hours
- GPU (A6000): ~2–3 hours
