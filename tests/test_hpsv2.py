"""
Unit tests for HPSv2 aesthetic scorer.
...
# Mock torch before importing HPSv2 if torch is not available
try:
    import torch
except ImportError:
    torch = MagicMock()
...
class TestHPSv2:
    """Tests for HPSv2 scorer functionality."""
    
    def test_initialization_auto_device(self):
        """Test HPSv2 initialization with auto device detection."""
        scorer = HPSv2(device="auto")
    ...
    def test_initialization_explicit_device(self):
        """Test HPSv2 initialization with explicit device."""
        scorer = HPSv2(device="cpu")
    ...
    @patch('src.scoring.hpsv2.AutoModel')
    @patch('src.scoring.hpsv2.AutoProcessor')
    ...
    def test_load_model_failure(self, mock_processor_cls, mock_model_cls):
        ...
        with pytest.raises(RuntimeError, match="HPSv2 loading failed"):
            scorer.load()
    ...
    def test_compare_returns_valid_range(
        ...
        # Mock model to return scores (single image, single logit)
        mock_output = MagicMock()
        mock_output.logits = torch.tensor([[2.5]])
    ...
    def test_compare_image_a_preference(
        ...
        # HPSv2 scores via model.get_image_features (image-only); image A has the
        # larger feature norm (= higher quality score).
    ...
    def test_compare_image_b_preference(
        ...
        # HPSv2 scores via model.get_image_features (image-only); image B has the
        # larger feature norm (= higher quality score).
        mock_model.get_image_features.side_effect = [
    ...
    def test_compare_equal_scores(
        ...
        # Both images return the same feature vector → diff = 0 → sigmoid(0) = 0.5
        equal_features = torch.tensor([[2.5]])
        mock_model.get_image_features.return_value = equal_features
    ...
    def test_batch_compare_multiple_pairs(
        ...
        # Mock outputs for 6 comparisons (3 pairs × 2 images)
        outputs = []
        for _ in range(6):
            mock_output = MagicMock()
            mock_output.logits = torch.tensor([[2.0 + torch.randn(1).item()]])
    ...
    def test_sigmoid_normalization(self):
        ...
        assert abs(HPSv2._sigmoid(0.0) - 0.5) < 1e-6
        
        # Test positive difference
        assert HPSv2._sigmoid(1.0) > 0.5
        assert HPSv2._sigmoid(10.0) > 0.99
        
        # Test negative difference
        assert HPSv2._sigmoid(-1.0) < 0.5
        assert HPSv2._sigmoid(-10.0) < 0.01
        
        # Test bounds (sigmoid(100.0) saturates to 1.0 in float64)
        assert 0.0 < HPSv2._sigmoid(-100.0) < 1.0
        assert 0.0 < HPSv2._sigmoid(100.0) <= 1.0
